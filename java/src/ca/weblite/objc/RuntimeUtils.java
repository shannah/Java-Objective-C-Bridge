 ...
 * 
 * @see <a href=
 *      "https://developer.apple.com/library/mac/#documentation/Cocoa/Reference/ObjCRuntimeRef/Reference/reference.html">Objective-C
 *      Runtime Reference</a>
 */
public class RuntimeUtils {
	 ...
	 * If it fails to load, then this flag will be false. Therefore, if this flag is
	 * false, you shouldn't try to use the the api at all.
	 */
	 ...
	 * Effectively this just calls peer.getPeer() and returns its value. A
	 * convenience method.
	 * 
	 * @param peer The peer object that is wrapping the Objective-C class object.
	...
	/**
	 * Returns the name of an objective C class specified by the given class
	 * pointer.
	 * 
	...
	/**
	 * A wrapper for the clsName() method given a Peerable object that wraps the
	 * class pointer. This will return the class name.
	 * 
	 * @param peer
	 * @return The class name of the objective-c class that is wrapped by the given
	 *         Peerable object.
	 * @see Runtime.class_getName()
	 */
	 ...
	 * @see <a href=
	 *      "http://developer.apple.com/library/ios/#documentation/cocoa/conceptual/objectivec/Chapters/ocSelectors.html">Objective-C
	 *      Selectors Reference</a>
	 */
	 ...
	 * @param args The arguments passed to the method. These are passed raw and will
	 *             not be coerced.
	 * @return The result of the message. This may be a pointer, if the message
	 *         returns a pointer, or a value, if the method returns a BOOL, long,
	 *         int, short, byte, or char. Note that you cannot use this method for
	 *         messages that return float and double values. For these, you
	 *         <em>must</em> use msgDouble(). This is because doubles and floats are
	 *         handled using a different underlying runtime function
	 *         (objc_msgSend_fpret). Alternatively you can use the variation of
	 *         msg() that takes coerceInput and coerceOutput boolean values, as this
	 *         will automatically choose the correct underlying message function
	 *         depending on the return type reported by the message signature.
	 * 
	 * @see msgDouble()
	 */
	 ...
	 * sending messages that return int-compatible numeric values. E.g. byte, bool,
	 * long, int, short. Do not use this if the message will return something else
	 * (like a float, double, string, or pointer).
	 * 
	 * @param receiver The target of the message.
	 * @param selector The selector for the message.
	 * @param args     The arguments passed to the message.
	 ...
	 * for sending messages that return boolean-compatible numeric values.
	 * Essentially any non-zero value is interpreted (and returned) as true. Zero
	 ...
	 * for messages that return a CString. Do not use this for messages that return
	 * numeric values or Pointers to objects (e.g. NSString). Use the msg() variant
	 * that takes coerceInputs and coerceOutputs parameters if you want NSStrings to
	 * be automatically converted to java Strings.
	 * 
	 * @param receiver The target of the message.
	 * @param selector The selector for the message.
	 * @param args     Arguments passed to the message.
	...
	/**
	 * Sends a message that returns a double value. This variant must be used if the
	 * method returns a float or a double. It doesn't actually wrap the primitive
	 * msg() method.. Rather it uses a different core Objective-C method,
	 * objc_msgSend_fpret().
	 * 
	 * @param receiver The target of the message.
	 * @param selector The selector for the message.
	...
	/**
	 * Sends a message with the option of coercing the inputs and outputs. This
	 * variant uses a higher level of abstraction than the standard msg() and
	 * msgXXX() methods. If coerceReturn and coerceArgs are set to true, then this
	 * method will convert the inputs from Java inputs to the corresponding
	 * Objective-C inputs. Further it will return values that are more appropriate
	 * for use in Java.
	 * 
	 * <p>
	 ...
	 * </p>
	 * 
	 * @param coerceReturn If true, then the return value will be mapped to an
	 *                     appropriate Java value using the TypeMapper class.
	 * @param coerceArgs   If true, then the inputs will be mapped from Java to
	 *                     appropriate C values using the TypeMapper class.
	 * @param receiver     The target of the message.
	 * @param selector     The selector for the message.
	 * @param args         The arguments to be passed in the message.
	 * @return The return value of the message. This may be a Pointer or a value
	 *         depending on the return type of the message.
	 * 
	 */
	public static Object msg(boolean coerceReturn, boolean coerceArgs, Pointer receiver, Pointer selector,
			Object... args) {
		...
		if (numArgs == 2 && numArgs != args.length + 2) {
			throw new RuntimeException("Wrong argument count.  The selector " + selName(selector) + " requires "
					+ (numArgs - 2) + " arguments, but received " + args.length);
		}
	 ...
	 * 
	 * @param signature The signature for a parameter using <a href=
	 *                  "https://developer.apple.com/library/mac/#documentation/Cocoa/Conceptual/ObjCRuntimeGuide/Articles/ocrtTypeEncodings.html">Objective-C
	 *                  type encodings</a>.
	 ...
	 * 
	 * @param messages
	 * @return The result of the last message sent. The results of the individual
	 *         messages will also be stored inside the message objects themselves.
	 * 
	 */
		...
		for (int i = 0; i < messages.length; i++) {
			...
			if (m.status == Message.STATUS_SKIPPED) {
				...
				try {
					...
					m.result = msg(coerceOutput, coerceInput, m.receiver, m.selector,
							m.args.toArray(new Object[m.args.size()]));
				} catch (Exception ex) {
					m.error = ex;
				}
	...
	/**
	 * Wraps the given value in the appropriate ByReference subclass according to
	 * the provided signature. This is a useful method in cases where you need to
	 * pass a parameter to a C-function by reference, but you don't necessarily know
	 * what type of data it is.
	 * 
	 * @param val       The value to be wrapped in a byReference.
	 * @param signature The signature (using <a href=
	 *                  "https://developer.apple.com/library/mac/#documentation/Cocoa/Conceptual/ObjCRuntimeGuide/Articles/ocrtTypeEncodings.html">Objective-C
	 *                  type encodings</a>) of the value.
	...
	public static ByReference getAsReferenceWrapper(Object val, String signature) {
		...
		case '^':
		default:
			//// System.out.println("Outputting pointer by reference for value "+val+" and
			//// signature "+signature);
			if (val == null) {
				try {
			...
			if (Pointer.class.isInstance(val)) {
				...
				throw new RuntimeException(
						"Don't know what to do for conversion of value " + val + " and signature " + signature);
			}
	...
	/**
	 * Registers a Java object with the Objective-C runtime so that it can begin to
	 * receive messages from it. This will create an Objective-C proxy that passes
	 * messages to the Recipient object. This step is automatically handled by the
	 * NSObject class inside its init() method.
	 * 
	 * @param client
	 * @return
	 */
	 ...
	 * This will return null if nsObject is not an WLProxy object that has been
	 * previously registered with a Recipient.
	 * 
	 * @param nsObject
	 * @return
	 */
